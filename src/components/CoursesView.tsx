...
export const CoursesView: React.FC<CoursesViewProps> = ({
  ...
  // User's most recent course for the "Continue Learning" top action
  const mostRecentCourse = useMemo(() => {
    // 1. Prioritize currently active or in-progress course
    const inProgress = learningPath.find(
      (item) => item.status === 'in-progress' || (item as any).status === 'active'
    );
    if (inProgress) {
      const c = REAL_WORLD_COURSES.find((rc) => rc.id === inProgress.courseId);
      if (c) return { course: c, item: inProgress };
    }
    // 2. Fallback to any course saved in learning path
    const anyItem = learningPath[0];
    if (anyItem) {
      const c = REAL_WORLD_COURSES.find((rc) => rc.id === anyItem.courseId);
      if (c) return { course: c, item: anyItem };
    }
    return null;
  }, [learningPath]);
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-white border border-slate-200 rounded-2xl p-6 sm:p-8 shadow-xs">
        ...
        {/* Continue Learning Resume Banner for User's Most Recent Course */}
        {mostRecentCourse && (
          <div className="mt-6 bg-gradient-to-r from-slate-900 via-indigo-950 to-indigo-900 rounded-2xl p-5 text-white flex flex-col sm:flex-row sm:items-center justify-between gap-4 border border-indigo-500/20 shadow-xs">
            <div className="flex items-center gap-4">
              ...
              <div className="space-y-0.5">
                ...
                <p className="text-xs text-indigo-200/80">
                  Skill: {mostRecentCourse.course.skillsGained[0]} • {mostRecentCourse.course.provider} • {mostRecentCourse.course.duration}
                </p>
              </div>
            </div>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {processedCourses.map(({ course, matchScore, matchReasons, isSkillGapCloser }) => {
              ...
              const pathItem = learningPath.find((item) => item.courseId === course.id);
              const courseState: 'Not Started' | 'In Progress' | 'Completed' = pathItem
              ...
              const isInProgress = courseState === 'In Progress';
              const currentProgress = isCompleted ? 100 : (pathItem?.progressPercentage ?? 45);

              return (
                <div
                  key={course.id}
                  className={`border rounded-2xl p-5 transition-all shadow-xs flex flex-col justify-between space-y-4 relative group ${
                    isCompleted
                      ? 'bg-emerald-50/20 border-emerald-300 ring-1 ring-emerald-200/50'
                      : isInProgress
                      ? 'bg-white border-indigo-300 ring-1 ring-indigo-200/50'
                ...
                >
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="p-2.5 rounded-xl bg-slate-50 border border-slate-100 space-y-1">
                      ...
                      <div className="flex flex-wrap gap-1">
                        {course.skillsGained.slice(0, 3).map((skill) => (
                          <span
                            key={skill}
                            className="px-2 py-0.5 rounded text-[11px] font-semibold bg-indigo-50 text-indigo-700 border border-indigo-100"
                          >
                            {skill}
                          </span>
                        ))}
                    ...
                    <div className="text-[11px] text-slate-600 flex items-center gap-1.5">
                      ...
                      <span>
                        {isCompleted
                          ? 'All syllabus modules completed and verified'
                          : isInProgress
                          ? 'Current lesson: Practical Labs & Practice Assessment'
                    ...
                    {course.certificationAvailable && (
                      <div className="bg-amber-50/70 border border-amber-200/80 rounded-lg p-2 flex items-start gap-2">
