...
- If you have backlogs: Prioritize upcoming clearing exams; meanwhile build open source contributions to qualify for off-campus opportunities.`;
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Chat Container */}
      <div className="bg-white border border-slate-200 rounded-2xl shadow-xs overflow-hidden flex flex-col h-[520px] sm:h-[600px]">
        {/* Messages Scroll Area */}
        <div className="flex-1 p-3.5 sm:p-5 overflow-y-auto space-y-3.5 sm:space-y-4 bg-slate-50/40">
          {messages.map((msg) => {
            ...
            return (
              ...
              >
                ...
                <div
                  className={`max-w-[85%] sm:max-w-2xl rounded-2xl px-3.5 sm:px-4 py-2.5 sm:py-3 text-xs leading-relaxed ${
                    isUser
        ...
        <div className="p-3 bg-white border-t border-slate-100 flex items-center gap-2 overflow-x-auto pb-2">
          ...
          {quickPrompts.map((q, idx) => (
            <button
              ...
              className="px-2.5 py-1 rounded-md text-[11px] font-medium bg-slate-50 text-slate-700 hover:bg-indigo-50 hover:text-indigo-700 border border-slate-200 hover:border-indigo-200 whitespace-nowrap transition-colors cursor-pointer shrink-0"
            >
              {q}
            </button>
          ))}
